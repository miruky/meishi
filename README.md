...
書体は「ゴシック・明朝・等幅」から、名刺の寄せ方は「左寄せ・中央」から選べる。OGPの寸法は横長(1200×630)・正方形(1200×1200)・ワイド(1600×900)を切り替えられ、構図が寸法に合わせて組み直される。
...
| OGP  | `0 0 1200 630` / `0 0 1200 1200` / `0 0 1600 900` | サイトの`og:image`の元 |

- 文字はパス化せず`text`要素のまま埋め込む。書体は選択(ゴシック・明朝・等幅)に応じた`font-family`を与え、実体は表示環境のシステムフォントに任せる。
- 空欄の項目は要素ごと出力しない。
- 文字列はすべてXMLエスケープされる。
- OGPの寸法を変えると、文字の大きさと配置が寸法に合わせて再計算される。
